...
<div id="ai-chat" aria-live="polite" aria-label="AI chat dialog">
  ...
  closeBtn.addEventListener('click', () => chat.classList.remove('open'));
    ...
    function addMessage (text, who='bot',extras={}){
      const div = document.createElement('div');
      div.className = `msg ${who}` + ( extras.thinking ? 'thinking': '');
      div.textContent =text;
      messages.appendChild(div);
      messages.scrollTop = messages.scrollHeight;
      return div;
    }
async function sendToAS(question)  {
  const res = await fetch('/api/chat',{
    method:'POST',
    headers: {'content-Type': 'application/json'},
    body: JSON.stringify({
    messages: [
      {roles:'user',content: question}
    ]  
    })
  })
...
fomr.addEventListener('submit', async (e) =>{
  e.preventDefault();
  ...
  const thinking = addMessage('Thinking...', 'bot',{thinking:true});
  try{
    const answer = await sendToAI(q);
    thinking.remove();
